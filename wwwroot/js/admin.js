...
function retrieveSetting(name) {
  switch (name) {
    ...
    case "delete-from-product":
      return document.getElementById("delete-images-from-products-checkbox")
        .checked;
      break;
...
function deleteProduct(productId) {
  confirmAction(
    ...
    () => {
      let deletionIndicator = SuopSnack.add(
        "Deleting...",
        Infinity,
        null,
        true
      ...
      fetch(`${window.location.origin}/api/products/${productId}`, {
        method: "delete",
        headers: { "delete-images": retrieveSetting("delete-product-images") },
      })
        .then((data) => data.text())
        .then((data) => {
          if (data == "deleted") {
...
function deleteImage(id) {
  confirmAction(
    "Are you sure you want to delete this image? This cannot be undone. Deleting images that are in use will cause broken pages.",
    () => {
      let deletionIndicator = SuopSnack.add(
        "Deleting...",
        Infinity,
      ...
      fetch(`${window.location.origin}/api/images`, {
        ...
        headers: {
          id: id,
          "delete-from-product": retrieveSetting("delete-from-product"),
        },
      })
        .then((data) => data.text())
        .then((data) => {
...
function popupForm(mode, data = {}) {
  ...
  function createFormNode() {
    ...
    switch (mode) {
      ...
      case formModes.editProduct:
        form.innerHTML = `<div><h1>${
          mode == formModes.createProduct ? "Create" : "Edit"
        } Product</h1></div>`;
        createInputNode("Name", "Name", data.name ?? "Example name");
        descriptionNode = createInputNode(
          "Description",
          "Description",
    ...
    form.innerHTML +=
      '<button type="submit" class="rounded-square-button">Submit</button>';
  ...
  function handleResponse(response) {
    switch (mode) {
      ...
      case formModes.editImage:
        ...
        document
          .querySelector("#image_" + data.id)
          .querySelector(".admin-description").innerHTML =
  ...
  function handleFailure(error) {
    switch (mode) {
      default:
        if (infoSnackbar == undefined) infoSnackbar = SuopSnack.add();
